    ...
    d3.netJsonGraph = function(url, opts) {
        ...
        opts = d3._extend({
            ...
            linkDistanceFunc: function(d){
                ...
                // if(d.source.linkCount >= 2 && d.target.linkCount >= 2) {
                //     return val * 2;
                return val;
            },

            chargeFunc: function(n){
              var val = opts.charge;
              if(n.linkCount < 10) {
                return -1000;
              }
            ...
            circleRadiusFunc: function(n){
              var val = opts.circleRadius;
              if (n.linkCount >= 30 ) {
                return 24;
              }
              else if (n.linkCount >= 20) {
                return 18;
              }
              return val;
            },
            ...
            onClickNode: function(n) {
                var overlay = d3.select(".njg-overlay"),
                    ...
                    if(n.linkCount) { html += "<p><b>Degree</b>: " + n.linkCount + "</p>"; }
                    if(n.properties) {
                        var attrs = ["Type", //string
                                     "Degree_centrality", // num
                                     "Closeness_centrality", // num
                                     "Betweenness_centrality", // num
                                     "Freq", // num
                                     "Most_friendly", //string
                                     "Polarity", //string
                                     "ES1", //string
                                     "ES2", //string
                                     "Score_ES1" // num
                        ...
                        for(var i in attrs){
                          var att = attrs[i];
                          if(typeof(n.properties[att]) === "string"){
                            if(att.substring(0,2) === "ES") {
                              if(n.properties[att]){
                                html += "<p><b>"+"Extractive summary"+att.substring(3, 2)+"</b>: " + n.properties[att] + "</p>";
                              }
                            }
                            else {
                              html += "<p><b>"+att.replace(/_/g, " ")+"</b>: " + n.properties[att] + "</p>";
                            }
                          }
                          else {
                            if(att === "Freq"){
                              html += "<p><b>"+"Frequency"+"</b>: " + Math.round(n.properties[att]*precision)/precision + "</p>";
                            }
                            else if(att === "Score_ES1") {
                              html += "<p><b>"+"Score of extractive summary1"+"</b>: " + Math.round(n.properties[att]*precision)/precision + "</p>";
                            }
                            else {
                              html += "<p><b>"+att.replace(/_/g, " ")+"</b>: " + Math.round(n.properties[att]*precision)/precision + "</p>";
                            }
                          }
                        } // end of for
                    }
        ...
        var el = d3.select(opts.el).style("position", "relative"),
            ...
            processJson = function(graph) {
                ...
                var attrs = ["Number_of_nodes",
                             "Number_of_edges",
                             "Number_of_subgraph",
                             "Maximum_degree_centrality_node",
                             "Maximum_degree_centrality",
                             "Maximum_betweenness_centrality_node",
                ...
                for(var i in attrs) {
                    var attr = attrs[i];
                    if(graph[attr]) {
                        html += "<p><b>" + attr.replace(/_/g," ") + "</b>: <span>" + graph[attr] + "</span></p>";
                    }
                }
